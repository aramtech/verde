...
export const readAnswerTo = lock_method(async (question: string, opts?: { type: "input" | "password" }) => {
...
export const readPrompt = lock_method(async (question: string, choices: string[]) => {
...
export const requestPermsToRunWithCb = lock_method(async (msg: string, cb: () => Promise<void> | void) => {
    return await spin_wrapper(async () => {
...
export const requestPermsToRun = lock_method(async (msg: string) => {
    return spin_wrapper(async () => {
        const answer = await readPrompt(msg, ["yes", "no"]);
